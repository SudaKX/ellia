 ...
 * 电源 / 会话控制菜单。
 *
 * 提供三个操作按钮，点击后向父组件抛出事件：
 * - `switchUser` — 切换登录账户
 * - `restart`    — 重启 FakeOS 会话
 * - `shutdown`   — 关闭 / 退出
 *
...
<template>
  <div class="power-menu">
    ...
    <div class="menu-body">
      <button class="menu-item" @click="$emit('switchUser')">
        <UserRound :size="14" :stroke-width="1.5" />
        <span>更改账户</span>
      </button>
      <button class="menu-item" @click="$emit('restart')">
        <RotateCw :size="14" :stroke-width="1.5" />
        <span>重启</span>
      </button>
      <button class="menu-item menu-item--danger" @click="$emit('shutdown')">
