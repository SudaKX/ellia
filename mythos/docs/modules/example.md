...
`test_example_module.py` 使用 FakeObjectStore 验证端到端行为、`act1_` token、对象键和重复提交；`test_example_rustfs_integration.py` 在启用 RustFS 时读取真实预签名 URL。开发页面 `/example/` 使用动态树端点展示该流程。
