...
Example 注册 validation ID `example-answer`。其 handler 规范化 `answer`，拒绝错误答案；首次正确提交推进进度并生成 recovery report；已完成时直接接受。它说明 validation 是模块语义命令入口，而非可由模块自行新增的 HTTP 回调。
