...
| 版本 | 来源 | 含义 |
| --- | --- | --- |
| `tree_version` | `GET /api/v1/files/version` | 冻结后的静态 FileTree Catalog 版本。文件 Node、revision、路径、下载名、媒体类型和 RustFS 对象版本变化时更新。 |
...
GET /api/v1/files?path=<absolute virtual path>
...
`GET /files?path=/docs` 返回该目录的直接可见子项：
...
FileService 当前不提供完整树快照。前端按目录缓存，每个目录缓存记录关联的 `tree_version` 和 `progress.version`。

已知路径不在本地缓存中时，直接请求 `GET /files?path=<path>`。返回 `404` 表示目录不存在或对当前玩家不可见；客户端不应尝试区分这两种情况。
