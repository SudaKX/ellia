...
Artifact generator 与 node generator 必须以 `module_handler(module)(revision)` 标记。该 callback ID 是模板版本与 Catalog 快照的一部分，改变装饰器所属模块、函数限定名或 revision 会触发对应 Artifact 重建。
