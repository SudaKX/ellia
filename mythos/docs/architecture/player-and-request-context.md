...
`PlayerInterfaces` 位图为 `PROGRESS`、`ARTIFACTS`、`ACCOUNTS`、`CREDITS`、`HINTS`、`TASKS` 和 `ALL`。`get_context()` 创建只读 `RequestContext`，文件动态端点加载全部 Interface，但 `pft4_` 只采集 Artifact 和文件 access_rule 声明的状态版本。`RequestContext` 保存 identity、Player 和 followup 收集器；`CommandContext` 增加 UUID `request_id` 与 `reject()`；`TaskContext` 提供任务时间、异常和 meta 状态。
