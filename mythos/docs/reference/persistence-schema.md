...
SQLite 是玩家和发布登记的权威数据源；RustFS/S3 保存文件字节，本地文件系统保存 checkpoint 与 Catalog 快照 JSON。当前 migration head 为 `0010_content_versions`，共有十四张表。对象存储 bucket versioning 已停用，VersionId 不进入 Mythos 持久化模型。
