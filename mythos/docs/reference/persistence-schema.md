...
SQLite 是玩家和发布登记的权威数据源；RustFS/S3 保存文件字节和版本，本地文件系统保存 checkpoint JSON。当前 migration head 为 `0005_player_artifacts`，共有九张表。
...
- Artifact 对象：`artifacts/{player_id}/{artifact_id}/{revision}/{digest}`；上传失败或 SQL 回滚后的孤儿由 `ArtifactCleanupService` 最佳努力清理。
- `RequestCache`：仅当前 Python 进程内存，受 TTL 和 maxsize 限制。

迁移顺序：`0001_initial_auth`、`0002_player_graph_progress`、`0003_remove_legacy_progress_fields`、`0004_static_file_registrations`、`0005_player_artifacts`。运行迁移时从 `mythos/` 使用 `python -m alembic -c alembic.ini upgrade head`。
