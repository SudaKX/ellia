...
所有业务 Router 使用 `/api/v1` 前缀；`GET /health` 不在此前缀下。除注册、登录和健康检查外，调用都需要 `Authorization: Bearer <access token>`。对象存储预签名 URL 不携带 Bearer header，前端必须先向 Mythos 获取 URL，再直接请求对象存储。
