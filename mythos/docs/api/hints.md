...
content URL 响应为 `{url, expires_at, content_token}`，带 token ETag、`Vary: Authorization` 与私有缓存策略。旧 token 返回 `412 hint-content-version-mismatch`；客户端刷新 Hint 列表后最多重试一次。预签名 URL 不应写入持久化前端状态。
