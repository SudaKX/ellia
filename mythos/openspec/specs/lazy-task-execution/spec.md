...
定义玩家任务注册、持久化、惰性触发执行、事务隔离、异常恢复、Hook 集成、快照对账和任务 HTTP API 的规范行为。
...
- **WHEN** TaskService 根据任务身份查找 Handler
- **THEN** TaskService SHALL 从冻结 TaskCatalog 读取该 Handler 当前声明的依赖并用于 Player Interface 加载
...
- **THEN** TaskService SHALL 保留旧的 `time_1`，并保存 Handler 设置的其他任务状态
...
- **THEN** Workflow SHALL 直接调用 TaskService 的事务内 `run_itx()` 入口，且不得依赖 HTTP CommandContext；TaskContext SHALL 使用 silent 或调用方提供的 ContextScope
...
- **THEN** TaskService SHALL 保持现有 Artifact 对象写入和 SQL 提交顺序，且 Operation transaction 不得覆盖 Task transaction 的已提交结果
...
- **THEN** Task command adapter SHALL 在 Task transaction 中处理任务，为本轮每个任务返回 `success` 或 `failure` 状态，并将 Followup 转换为响应 JSON
